...
public class StandardLogger {
    ...
    @Pointcut(
            "execution(* com.pyokemon..api..*Service.*(..)) || execution(* com.pyokemon.common.util.*.*(..))")
    ...
    public void writeResponseLog(
            MultiReadHttpServletResponse httpSvtResp, String loggerName, String accept) {
        try {
            /* Response String 추출 */
            HttpServletRequest request = getCurrentRequest();
            ...
            this.writeLog(
                    loggerName,
                    String.format(
                            "[%s] Response > %s", request.getRequestURI(), responseObj.toString()));
            setGeneralMetadata();
        } catch (IOException e) {
        ...
        try {
            if (org.springframework.util.StringUtils.hasText(responseStr)) {
                ...
                try {
                    ...
                    /*
                     * Parsing Error 발생시에 다른 Type으로 한번도 Parsing 시도 후에도 Exception이
                     * 발생하면 로깅 Fail
                     */
