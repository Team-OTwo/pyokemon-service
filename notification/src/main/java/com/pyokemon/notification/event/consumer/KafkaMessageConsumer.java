...
public class KafkaMessageConsumer {
  ...
  void handleNotificationEvent(BookingEvent bookingEvent) {
    ...
    if (userInfo == null || !userInfo.getIsLogin() || userInfo.getFcmToken() == null) {
      log.warn("User info invalid or user not logged in, accountId: {}",
          bookingEvent.getAccountId());
      return;
    }

    EventInfoResponseDto eventInfo =
        getEventInfo(bookingEvent.getEventScheduleId(), bookingEvent.getSeatId());
    if (eventInfo == null) {
   ...
   * @return 생성된 알림 메시지
   */
  private NotificationSendRequestDto createNotificationMessage(NotificationType type,
      UserInfoResponseDto userInfo, EventInfoResponseDto eventInfo) {
    ...
    switch (type) {
      ...
      case CANCELLED:
        ...
        message.setMessage(String.format("%s님, %s 공연 예매가 취소되었습니다.", userInfo.getName(),
      ...
      case PAYMENT_FAILED:
        ...
        message.setMessage(String.format("%s님, %s 공연 예매 결제가 실패했습니다. 결제 정보를 확인해주세요.",
            userInfo.getName(), eventInfo.getEvent().getEvent_title()));
        break;
  ...
  private String buildBookedMessage(String userName, EventInfoResponseDto eventInfo) {
    return String.format(
        "%s님, 꿈에 그리던 %s 티켓 예매에 성공하셨어요!\n\n" + "잊지 못할 공연이 될 거예요.\n" + "・ 공연: %s\n" + "・ 일시: %s\n"
            + "・ 좌석: %s석 (%s층 %s열 %s석)\n\n" + "공연 당일, 설레는 마음으로 만나요!",
        userName, eventInfo.getEvent().getEvent_title(), eventInfo.getEvent().getEvent_title(),
  ...
  private EventInfoResponseDto getEventInfo(Long scheduleId, Long seatId) {
    try {
      ...
      log.error("Failed to get event info for scheduleId: {} and seatId: {}", scheduleId, seatId,
          e);
      return null;
    }
  }
