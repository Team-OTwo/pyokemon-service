...
public class ReceiveInvitationRequest implements AcaPyRequest {
  ...
  public static ReceiveInvitationRequest fromInvitation(Invitation invitation) {
    return ReceiveInvitationRequest.builder().type(invitation.getType()).id(invitation.getId())
        .label(invitation.getLabel()).handshakeProtocols(invitation.getHandshakeProtocols())
        .services(invitation.getServices()).useDidMethod(AcaPyConstants.DidMethod.PEER2).build();
  }
}
