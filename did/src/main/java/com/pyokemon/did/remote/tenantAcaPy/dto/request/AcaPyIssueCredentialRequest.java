...
public class AcaPyIssueCredentialRequest {
  ...
  public static AcaPyIssueCredentialRequest of(String connectionId, String bookingId,
    ...
    return AcaPyIssueCredentialRequest.builder().connectionId(connectionId).comment("예매 VC 발급")
        .autoOffer(
            true)
        .filter(
            Filter.builder()
                .ldProof(
                    LdProof.builder()
                        .credential(Credential.builder()
                            ...
                            .id(bookingId).type(List.of("VerifiableCredential")).issuer(issuerDid)
                            .issuanceDate(java.time.Instant.now().toString())
                            .credentialSubject(CredentialSubject.builder().id(subjectDid)
                                .bookingId(bookingId).build())
                            .build())
                        .options(Options.builder().proofType("Ed25519Signature2020").build())
                        .build())
                .build())
        .build();
  }
