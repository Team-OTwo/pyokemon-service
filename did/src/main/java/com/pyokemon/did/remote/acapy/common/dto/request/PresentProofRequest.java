...
public class PresentProofRequest implements AcaPyRequest {
  ...
  public static PresentProofRequest forTicketVerification(String challenge, String userPublicDid,
    ...
    Constraints constraints =
        Constraints.builder().fields(List.of(issuerField, evidenceField)).build();

    // 3. 입력 디스크립터 생성
    InputDescriptor descriptor = InputDescriptor.builder().id("ticket_verification")
        .schema(Collections
            .singletonList(new Schema("https://www.w3.org/2018/credentials#VerifiableCredential")))
    ...
    // 5. 옵션 생성
    Options options =
        Options.builder().challenge(challenge).domain(AcaPyConstants.Domain.PYOKEMON).build();

    // 6. DIF 생성
    Dif dif = Dif.builder().options(options).presentationDefinition(definition).build();

    // 7. 프레젠테이션 요청 생성
    PresentationRequest request = PresentationRequest.builder().dif(dif).build();

    // 8. 최종 요청 생성
    return PresentProofRequest.builder().autoRemove(false).presentationRequest(request).build();
  }
  ...
  private static Field createField(String path, String constValue) {
    return Field.builder().path(Collections.singletonList(path))
        .filter(new Filter("string", constValue)).build();
  }
}
