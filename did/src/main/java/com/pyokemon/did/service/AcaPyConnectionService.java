...
public interface AcaPyConnectionService {
   ...
   * <p>
   * 이 메서드는 Out-of-Band 초대장이 수락되어 연결이 활성화(active)될 때 호출됩니다.
   * 최초 초대 메시지 ID({@code inviMsgId})를 사용하여 보류 중인 연결 레코드를 찾고,
   * 최종적으로 확정된 연결 ID({@code connectionId})를 저장합니다.
   * 또한 연결 상태를 'ACTIVE' 로 변경하여 연결을 완료합니다.
   *
   * @param inviMsgId 최초 Out-of-Band 초대 메시지에 포함된 고유 식별자.
   * 이 ID를 통해 업데이트할 대상 연결 레코드를 조회합니다.
   * @param connectionId 연결이 활성화되면서 최종적으로 부여된 고유 연결 식별자.
   * @throws RetryException {@code inviMsgId}에 해당하는 연결 레코드를 즉시 찾을 수 없을 때 발생합니다.
   * 이는 웹훅 처리 순서나 네트워크 지연으로 인한 일시적인 상태일 수 있으므로,
   * 재시도를 통해 해결될 수 있습니다.
   */
