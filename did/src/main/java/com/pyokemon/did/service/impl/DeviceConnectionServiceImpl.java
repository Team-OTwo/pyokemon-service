...
public class DeviceConnectionServiceImpl implements DeviceConnectionService {
        ...
        try {
            ...
            if (shouldCreateInvitation) {
                ...
                AcaPyCreateInvitationRequest userRequest = AcaPyCreateInvitationRequest.of(
                        "Invitation to Credo from User ACA-Py", userId, deviceId);
                userAcapyResponse = remoteUserAcaPyService.acaPyCreateInvitation(authorization, userRequest);
                ...
                AcaPyCreateInvitationRequest mediatorRequest = AcaPyCreateInvitationRequest.of(
                        "Invitation to Credo from Mediator ACA-Py", userId, deviceId);
    ...
    private boolean processDeviceConnectionBusinessLogic(Long userId, String deviceId) {
        ...
        if (!existing.getDeviceId().equals(deviceId)) {
            ...
            log.info("기존 연결 폐기: id={}, oldDeviceId={}, newDeviceId={}", 
                    existing.getId(), existing.getDeviceId(), deviceId);
     ...
     * 기존 연결의 상태에 따라 처리합니다.
     */
    private boolean handleExistingConnectionByStatus(DeviceConnection existing, Long userId, String deviceId, String userAlias) {
        switch (existing.getStatus()) {
          case ACTIVE, DID_RECEIVED:
                log.error("이미 연결된 상태: userId={}, deviceId={}, alias={}", userId, deviceId, userAlias);
                throw new BusinessException("이미 연결되어 있는 connection이 active 상태입니다", DidErrorCodes.CONNECTION_ALREADY_ACTIVE);
                
            case INVITATION_SENT:
                log.error("이미 pending 중인 invitation: userId={}, deviceId={}, alias={}", userId, deviceId, userAlias);
                throw new BusinessException("이미 pending 중인 invitation이 존재합니다", DidErrorCodes.INVITATION_ALREADY_SENT);
                
            case REVOKED:
            default:
                log.info("기존 연결 재생성: status={}, alias={}, id={}",
                        existing.getStatus(), userAlias, existing.getId());
     ...
     */
    private void createNewDeviceConnection(Long userId, String deviceId, String userAlias, String reason) {
        DeviceConnection newConnection = DeviceConnection.builder()
                .connectionId(null)
                .deviceId(deviceId)
                .userId(userId)
                .alias(userAlias)
