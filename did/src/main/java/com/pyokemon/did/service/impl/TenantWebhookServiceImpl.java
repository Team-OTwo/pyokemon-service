...
public class TenantWebhookServiceImpl implements TenantWebhookService {
    ...
    @Transactional
    @Retryable(
            retryFor = {RetryException.class, DataAccessException.class, IOException.class},
    ...
    public void handleTenantConnectionWebhook(HandleTenantConnectionsRequest request) {
        ...
        AcaPyConnection connection = acaPyConnectionRepository.findByInviMsgId(request.getInvitationMsgId())
                .orElseThrow(() -> {
     ...
     */
    @Recover
    public void recoverTenantConnectionWebhook(Exception e, HandleTenantConnectionsRequest request) throws BusinessException {
        log.error("[Webhook] 재시도 실패 (Exception): msgId={}, error={}", 
                request.getInvitationMsgId(), e.getMessage());
    ...
    public void handleTenantOOBWebhook(HandleTenantOOBRequest handleTenantOOBRequest) {
        log.info("oob_id: {}, state: {}", handleTenantOOBRequest.getOobId(), handleTenantOOBRequest.getState());
    }
}
