...
public class TenantWebhookServiceImpl implements TenantWebhookService {
    ...
    private void updateVcCredentialExchangeId(IssueCredentialWebhookDto webhookDto) {
        ...
        try {
            ...
            String bookingIdStr = webhookDto.getByFormat().getCredOffer().getLdProof().getCredential().getCredentialSubject().getBookingId();
    ...
    private void updateVcStatus(String credExId, String credentialId, IssuedVc.VcStatus status) {
        log.info("VC 상태 업데이트 - credExId: {}, credentialId: {}, status: {}", credExId, credentialId, status);
        ...
        issuedVcRepository.update(issuedVc);
        log.info("VC 상태 업데이트 완료 - bookingId: {}, status: {}, credentialId: {}", 
                issuedVc.getBookingId(), status, credentialId);
    }
}
