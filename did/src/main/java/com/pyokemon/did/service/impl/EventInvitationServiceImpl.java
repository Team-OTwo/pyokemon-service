...
public class EventInvitationServiceImpl implements EventInvitationService {
  ...
  public CreateMediatorInvitationResponse getMediatorInvitation() {
    try {
      ...
      throw new BusinessException("미디에이터 초대장 생성에 실패했습니다.", DidErrorCodes.INVITATION_CREATION_FAILED,
          e);
    }
  }
  ...
  public void createEventInvitation(CreateEventInvitationRequest request) {
    try {
      ...
      WalletMetadata walletMetadata =
          walletMetadataRepository.findByTenantId(request.getTenantId()).orElseThrow(
              () -> new BusinessException("테넌트 지갑이 존재하지 않습니다.", DidErrorCodes.WALLET_NOTFOUND));
      ...
      AcaPyCreateInvitationResponse invitationResponse = remoteTenantAcaPyService.createInvitation(
          authorization, AcaPyCreateInvitationRequest.generate(request.getEventId()));

      log.info("이벤트 ID: {}, 테넌트 ID: {}에 대한 초대장이 생성되었습니다. OOB ID: {}", request.getEventId(),
          request.getTenantId(), invitationResponse.getOobId());

      // 이벤트 초대장 정보 저장
      eventInvitationRepository
          .save(invitationResponse.toEntity(request.getEventId(), request.getTenantId()));
