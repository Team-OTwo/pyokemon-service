...
/**
 * DID 서비스의 /api 경로에서 Gateway를 통해 전달받은 헤더 정보를 안전하게 추출하는 유틸리티 클래스
 * /api 경로는 Gateway를 거쳐서 사용자 인증 정보를 받아옵니다.
 */
public class GatewayRequestHeaderUtils {
    ...
    public static Long getUserIdOrThrowException() {
        ...
        try {
            ...
            throw new BusinessException("사용자 ID가 올바른 숫자 형식이 아닙니다: " + accountId, DidErrorCodes.ACCESS_DENIED);
        }
    }
    ...
    /**
     * Gateway에서 전달받은 사용자 ID를 반환합니다.
     * role이 USER인 경우에만 반환합니다.
     * @return 사용자 ID
