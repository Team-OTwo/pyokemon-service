...
public class TestController {
  ...
  public ResponseEntity<ResponseDto<String>> createTenantWallet() {
    try {
      ...
      return ResponseEntity.badRequest()
          .body(ResponseDto.error("테넌트 월렛 생성 실패: " + e.getMessage(), "WALLET_CREATION_FAILED"));
    }
  }
  ...
  public ResponseEntity<ResponseDto<String>> createUserWallet() {
    try {
      ...
      return ResponseEntity.badRequest()
          .body(ResponseDto.error("사용자 월렛 생성 실패: " + e.getMessage(), "WALLET_CREATION_FAILED"));
    }
  }
  ...
  public ResponseEntity<ResponseDto<String>> testCreateInvitations() {
    try {
      ...
      return ResponseEntity.badRequest()
          .body(ResponseDto.error("초대장 생성 실패: " + e.getMessage(), "INVITATION_CREATION_FAILED"));
    }
  }
  ...
  public ResponseEntity<ResponseDto<String>> testCreateVerificationUrl() {
    try {
      ...
      return ResponseEntity.badRequest().body(
          ResponseDto.error("검증 URL 생성 실패: " + e.getMessage(), "VERIFICATION_CREATION_FAILED"));
    }
  }
  ...
  /**
   * 실제 Credo ACA-Py에서 보내는 Connection Webhook을 받습니다. 이 엔드포인트는 Credo ACA-Py가 직접 호출합니다.
   */
  @PostMapping("/webhook/connection")
  public ResponseEntity<String> receiveConnectionWebhook(
      @RequestBody ConnectionWebhookRequest webhook) {
    try {
  ...
  /**
   * 실제 Credo ACA-Py에서 보내는 Issue Credential Webhook을 받습니다. 이 엔드포인트는 Credo ACA-Py가 직접 호출합니다.
   */
  @PostMapping("/webhook/issue-credential")
  public ResponseEntity<String> receiveIssueCredentialWebhook(
      @RequestBody IssueCredentialWebhookRequest webhook) {
    try {
  ...
  /**
   * 실제 Credo ACA-Py에서 보내는 Present Proof Webhook을 받습니다. 이 엔드포인트는 Credo ACA-Py가 직접 호출합니다.
   */
  @PostMapping("/webhook/present-proof")
  public ResponseEntity<String> receivePresentProofWebhook(
      @RequestBody PresentProofWebhookRequest webhook) {
    try {
  ...
  /**
   * 실제 Credo ACA-Py에서 보내는 Basic Message Webhook을 받습니다. 이 엔드포인트는 Credo ACA-Py가 직접 호출합니다.
   */
  @PostMapping("/webhook/basic-message")
  public ResponseEntity<String> receiveBasicMessageWebhook(
      @RequestBody BasicMessageWebhookRequest webhook) {
    try {
  ...
  /**
   * 실제 Credo ACA-Py에서 보내는 Out of Band Webhook을 받습니다. 이 엔드포인트는 Credo ACA-Py가 직접 호출합니다.
   */
  @PostMapping("/webhook/out-of-band")
  public ResponseEntity<String> receiveOutOfBandWebhook(
      @RequestBody OutOfBandWebhookRequest webhook) {
    try {
  ...
  /**
   * 실제 Credo ACA-Py에서 보내는 LD Proof Webhook을 받습니다. 이 엔드포인트는 Credo ACA-Py가 직접 호출합니다.
   */
  ...
  public ResponseEntity<ResponseDto<String>> testHandleVerification(
      @PathVariable String pres_ex_id) {
    try {
      // Gateway 헤더에서 계정 정보 추출
      Long accountId = GatewayRequestHeaderUtils.getUserIdOrThrowException();
      ...
      return ResponseEntity.badRequest()
          .body(ResponseDto.error("검증 처리 실패: " + e.getMessage(), "VERIFICATION_HANDLING_FAILED"));
    }
  }
  ...
  public ResponseEntity<ResponseDto<Map<String, Object>>> getTestStatus() {
    try {
      Map<String, Object> status = Map.of("service", "DID Service Test Controller", "timestamp",
          System.currentTimeMillis(), "availableTests", Map.of("wallet", "월렛 생성 테스트", "invitations",
              "초대장 생성 테스트", "verifications", "검증 URL 생성 테스트", "webhooks", "Webhook 시뮬레이션 테스트"));

      return ResponseEntity.ok(ResponseDto.success(status, "테스트 컨트롤러 상태 조회 성공"));
    } catch (Exception e) {
      log.error("상태 조회 실패: {}", e.getMessage(), e);
      return ResponseEntity.badRequest()
          .body(ResponseDto.error("상태 조회 실패: " + e.getMessage(), "STATUS_QUERY_FAILED"));
    }
  }
}
