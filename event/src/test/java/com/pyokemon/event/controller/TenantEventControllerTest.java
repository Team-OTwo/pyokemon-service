...
class TenantEventControllerTest {
    ...
    void getTenantEventList_ShouldReturnTenantEventList() throws Exception {
        ...
        mockMvc.perform(get("/api/events/tenant")
    ...
    void getTenantMonthlySummary_ShouldReturnMonthlySummary() throws Exception {
        ...
        when(tenantEventService.getMonthlyEventSummary(accountId, year, month)).thenReturn(mockMonthlySummary);

        mockMvc.perform(get("/api/events/tenant/monthly-summary")
                .param("account_id", accountId.toString())
    ...
    void getTenantEventDetail_ShouldReturnTenantEventDetail() throws Exception {
        ...
        when(tenantEventService.getTenantEventDetailByEventId(eventId)).thenReturn(mockTenantEventDetail);

        mockMvc.perform(get("/api/events/tenant/{eventId}/detail", eventId))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.success").value(true))
    ...
    void getTenantBookingDetail_ShouldReturnTenantBookingDetail() throws Exception {
        ...
        when(tenantEventService.getTenantBookingDetailByEventScheduleId(eventScheduleId)).thenReturn(mockTenantBookingDetail);
    ...
    void updateEvent_ShouldUpdateEventSuccessfully() throws Exception {
        EventRegisterDto updateDto = EventRegisterDto.builder()
                .title("Updated Event")
                .ageLimit(19L)
        ...
        EventResponseDto updatedResponse = EventResponseDto.builder()
                .eventId(1L)
                .title("Updated Event")
                .build();
        ...
        mockMvc.perform(put("/api/events/tenant/{eventId}", 1L)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void registerEventSchedule_ShouldRegisterEventScheduleSuccessfully() throws Exception {
        Long eventId = 1L;
        EventScheduleDto scheduleDto = EventScheduleDto.builder()
                .venueId(1L)
                .ticketOpenAt(LocalDateTime.now().plusDays(1))
                .eventDate(LocalDateTime.now().plusDays(7))
                .build();
        ...
        mockMvc.perform(post("/api/events/tenant/{eventId}/schedules", eventId)
                .contentType(MediaType.APPLICATION_JSON)
