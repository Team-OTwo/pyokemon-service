...
class BookingSeatServiceTest {
  ...
  void createBooking_AlreadyBookedSeat_ThrowsException() {
    ...
    when(eventScheduleRepository.findById(validEventScheduleId))
        .thenReturn(Optional.of(validEventSchedule));
    when(seatRepository.findById(validSeatId)).thenReturn(Optional.of(validSeat));
    when(bookingRepository.findByEventScheduleIdAndStatus(validEventScheduleId,
        Booking.Booked.BOOKED)).thenReturn(Arrays.asList(bookedBooking));
  ...
  void createBooking_PendingSeat_ThrowsException() {
    ...
    when(seatRepository.findById(validSeatId)).thenReturn(Optional.of(validSeat));
    when(bookingRepository.findByEventScheduleIdAndStatus(validEventScheduleId,
        Booking.Booked.BOOKED)).thenReturn(Collections.emptyList());
    when(bookingRepository.findByEventScheduleIdAndStatus(validEventScheduleId,
        Booking.Booked.PENDING)).thenReturn(Arrays.asList(pendingBooking));
  ...
  void createBooking_ExistingBookingForEvent_ThrowsException() {
    ...
    when(bookingRepository.findByEventScheduleIdAndStatus(validEventScheduleId,
        Booking.Booked.BOOKED)).thenReturn(Collections.emptyList());
    when(bookingRepository.findByEventScheduleIdAndStatus(validEventScheduleId,
        Booking.Booked.PENDING)).thenReturn(Collections.emptyList());
    when(bookingRepository.findByEventScheduleIdAndAccountId(validEventScheduleId, validAccountId))
  ...
  void getEventScheduleSeats_Success() {
    ...
    when(eventScheduleRepository.findById(validEventScheduleId))
        .thenReturn(Optional.of(validEventSchedule));
    when(seatRepository.findByVenueId(validVenueId)).thenReturn(seats);
    when(bookingRepository.findByEventScheduleIdAndStatus(validEventScheduleId,
        Booking.Booked.BOOKED)).thenReturn(Collections.emptyList());
    when(bookingRepository.findByEventScheduleIdAndStatus(validEventScheduleId,
        Booking.Booked.PENDING)).thenReturn(Collections.emptyList());
    when(priceRepository.findByEventScheduleId(validEventScheduleId)).thenReturn(prices);
    when(seatClassRepository.findAll()).thenReturn(seatClasses);

    EventScheduleSeatResponse result =
  ...
  void getEventScheduleSeats_InvalidEventScheduleId_ThrowsException() {
    BusinessException exception =
        assertThrows(BusinessException.class, () -> bookingSeatService.getEventScheduleSeats(null));
  ...
  void getSeatMapOnlyByGrade_Success() {
    ...
    List<SeatMapDetail> result =
        bookingSeatService.getSeatMapOnlyByGrade(validEventScheduleId, "VIP");
  ...
  void deletePendingBookings_Success() {
    ...
    when(bookingRepository.findByStatus(Booking.Booked.PENDING)).thenReturn(pendingBookings);
  ...
  void deletePendingBookings_ExceptionDuringDeletion_LogsError() {
    List<Booking> pendingBookings = Arrays.asList(validBooking);
    when(bookingRepository.findByStatus(Booking.Booked.PENDING)).thenReturn(pendingBookings);
    doThrow(new RuntimeException("Database error")).when(bookingRepository).delete(validBooking);
