...
class EventServiceTest {
  ...
  void 공연상세정보조회_비회원() {
    ...
    EventDetailResponseDTO mockDto =
        EventDetailResponseDTO.builder().eventId(eventId).title("테스트 공연").ageLimit(15L)
            .description("공연 설명입니다").genre("뮤지컬").thumbnailUrl("http://image.url")
            .eventScheduleId(100L).ticketOpenAt(LocalDateTime.of(2025, 8, 10, 10, 0))
            .eventDate(LocalDateTime.of(2025, 8, 15, 19, 30)).venueName("서울아트센터").isSaved(false)
            .build();
