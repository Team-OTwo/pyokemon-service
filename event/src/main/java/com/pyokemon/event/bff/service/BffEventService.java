...
public class BffEventService {
    ...
    public List<BffVenueDto> getVenues(List<Long> ids){
        ...
        Map<Long, BffVenueDto> byId = rows.stream()
                .collect(Collectors.toMap(BffVenueDto::getVenueId, Function.identity()));
    ...
    public List<BffEventDto> getEvents(List<Long> ids){
        ...
        // ID 기준 Map으로 정리
        Map<Long, BffEventDto> byId = rows.stream()
                .collect(Collectors.toMap(BffEventDto::getEventId, Function.identity()));
