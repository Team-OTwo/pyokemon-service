...
public class TenantService {
    ...
    private Tenant findByLoginId(String loginId) {
        ...
        return tenantRepository.findByLoginId(loginId)
            .orElseThrow(TenantException::notFound);
    }
    ...
    public TenantDetailResponseDto updateProfile(Long tenantId, UpdateProfileRequestDto request) {
        ...
        Tenant updatedTenant = Tenant.builder()
            ...
            .loginId(existingTenant.getLoginId())        // 수정 불가
    ...
    public void changePassword(Long tenantId, UpdatePasswordRequestDto request) {
        ...
        Tenant updatedTenant = Tenant.builder()
            .id(existingTenant.getId())
            .loginId(existingTenant.getLoginId())
            .password(encodedNewPassword)                 // 새 비밀번호
            .corpName(existingTenant.getCorpName())
            .corpId(existingTenant.getCorpId())
            .city(existingTenant.getCity())
            .street(existingTenant.getStreet())
            .zipcode(existingTenant.getZipcode())
            .ceoName(existingTenant.getCeoName())
    ...
    public String login(LoginRequestDto request) {
        ...
        // 4. JWT 토큰 생성 및 반환
        return jwtTokenProvider.generateToken(tenant.getLoginId(), "TENANT");
    }

    //로그아웃
    public void logout(String token) {
        // JWT 토큰 기반 로그아웃
        // Gateway에서 토큰 검증을 담당하므로 여기서는 별도 처리가 필요 없을 수 있음
        // 필요시 토큰 블랙리스트나 Redis 캐시 무효화 등의 로직을 추가
        
        if (token == null || token.trim().isEmpty()) {
            throw new TenantException("토큰은 필수입니다", "TOKEN_REQUIRED");
        }
        
        // 토큰 유효성 검증
        if (!jwtTokenProvider.validateToken(token)) {
            throw new TenantException("유효하지 않은 토큰입니다", "INVALID_TOKEN");
        }
        
        // TODO: 필요시 토큰 블랙리스트 추가 등의 로직 구현
        // 현재는 클라이언트에서 토큰을 삭제하는 것으로 로그아웃 처리
    }
