...
public class TenantMapper {
  ...
  public Tenant toEntity(CreateTenantRequestDto request, String encodedPassword) {
    ...
    return Tenant.builder().loginId(request.getLoginId()).password(encodedPassword)
        ...
        .city(parseCity(request.getCity())).street(parseStreet(request.getStreet()))
        .zipcode(parseZipcode(request.getZipcode())).ceoName(request.getCeoName()).build();
  }
  ...
  /**
   * 주소에서 우편번호 추출 (현재는 빈 문자열 반환) 실제로는 우편번호 API나 별도 필드로 받아야 함
   */
